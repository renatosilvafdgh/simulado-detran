...
async function createBlogPost() {
    const postData = {
        title: "Guia de Placas de Trânsito CONTRAN: O que Você PRECISA Saber para não Errar",
        content: `# O Código Visual das Vias: Como a Sinalização Vertical Governa o Espaço Público
...
*   **R-1 (Pare):** Única placa de formato octogonal. Sua geometria exclusiva visa garantir o reconhecimento imediato pelo condutor, mesmo quando visualizada por trás ou sob condições adversas de visibilidade, assegurando que o motorista pare totalmente o veículo.
*   **R-19 (Velocidade Máxima Permitida):** Estabelece o limite legal intransponível para o trecho. 
*   **R-6a (Proibido Estacionar):** Uma restrição de uso da via que, se descumprida em desacordo com a regulamentação, caracteriza infração grave.
...
*   **A-14 (Semáforo à frente):** Incorpora as cores verde, amarelo e vermelho para associação cognitiva imediata.
*   **A-24 (Obras):** Para destacar o caráter temporário e o risco elevado, tanto o **fundo quanto a orla externa** devem ser obrigatoriamente na cor laranja.
*   **A-30c (Trânsito Compartilhado):** Sinalização especial que adverte ciclistas e pedestres sobre a circulação conjunta, vital para a mobilidade urbana segura.
...
*   **Serviços Auxiliares:** Sinais como o **S-2 (Hospital)** e **S-1 (Estacionamento)** garantem que o usuário localize suporte essencial sem desviar sua atenção da condução segura.
...
A sinalização vertical não é uma sugestão; é o manual de instruções da via. Respeitá-la é a forma mais eficaz de garantir que todos cheguem ao seu destino com segurança.`,
        meta_title: "Guia de Placas de Trânsito CONTRAN: Manual Completo 2022",
        meta_description: "Domine as placas de regulamentação e advertência do CONTRAN. Entenda as regras técnicas, evite multas e garanta sua segurança com este guia especializado.",
        hashtags: "ManualCONTRAN, LegislacaoDeTransito, SinalizacaoViaria, SegurancaNoTransito, CNH2024"
    }
    ...
    if (existing) {
        console.log('⚠️  Slug já existe! Adicionando timestamp...')
    ...
    const { data, error } = await supabase
        ...
        .insert({
            ...
            meta_title: postData.meta_title.substring(0, 60),
            meta_description: postData.meta_description.substring(0, 155),
            hashtags: postData.hashtags,
            published_at: new Date().toISOString(),
            author: 'Simulado Brasil',
            og_image: null // Será adicionado depois
        })
        .select()
    ...
    console.log('\n💡 Próximo passo: Adicione uma imagem de capa ao campo og_image')
}
