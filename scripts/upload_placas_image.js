...
async function uploadImageAndUpdatePost() {
    const imagePath = path.join(process.cwd(), 'public', 'images', 'placas-transito.jpg')

    console.log('📤 Fazendo upload da imagem para Supabase Storage...')

    // Ler o arquivo
    const imageBuffer = fs.readFileSync(imagePath)
    const fileName = `blog/placas-contran-${Date.now()}.jpg`

    // Upload para o bucket 'blog-images' (você pode precisar criar este bucket primeiro)
    ...
    if (uploadError) {
        ...
        console.log('💡 Dica: Certifique-se de que o bucket "blog-images" existe no Supabase Storage')

        // Usar a imagem local como fallback
        const localImageUrl = 'https://souhabilitado.com/images/placas-transito.jpg'
        await updatePostContent(localImageUrl)
        return
    }
...
![Placas de Regulamentação e Advertência](${imageUrl})
...
*   **Serviços Auxiliares:** Sinais como o **S-2 (Hospital)** 🏥 e **S-1 (Estacionamento)** 🅿️ garantem que o usuário localize suporte essencial sem desviar sua atenção da condução segura.
...
A sinalização vertical não é uma sugestão; é o manual de instruções da via. Respeitá-la é a forma mais eficaz de garantir que todos cheguem ao seu destino com segurança.`
    ...
    console.log('✅ Post atualizado com a imagem das placas!')
    console.log('\n🔗 Visualize: https://souhabilitado.com/blog/' + slug)
}
